...
TARGET = pathlib.Path('src/webui/app.py')

# frozen from TASK.md -- re-run `ollama-dispatch-scaffold --freeze-literals .`
LITERALS = [[None, 'from ..webui.scheduled_routes import create_router, scheduled_search_loop'], [None, 'scheduled_routes = create_router(templates)'], [None, 'app.include_router(scheduled_routes)'], [None, 'scheduled_task = asyncio.create_task(scheduled_search_loop())'], [None, 'scheduled_task.cancel()']]
