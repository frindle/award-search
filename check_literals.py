...
TARGET = pathlib.Path('src/webui/scheduled_routes.py')

# frozen from TASK.md -- re-run `ollama-dispatch-scaffold --freeze-literals .`
LITERALS = [[None, 'TEMPLATES.TemplateResponse('], [None, 'def scheduled_templates(request: Request):'], [None, 'def scheduled_partners(request: Request):'], [None, 'def scheduled_partner(request: Request, code: str):'], [None, 'def scheduled_run(request: Request, body: RunRequest):'], [None, '"partner not found"'], [None, '"program is required"']]
