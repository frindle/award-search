...
TARGET = pathlib.Path('src/scheduled_searches.py')

# frozen from TASK.md -- re-run `ollama-dispatch-scaffold --freeze-literals .`
LITERALS = [[None, "DATA_DIR = Path('data')"], [None, "SCHEDULES_FILE = DATA_DIR / 'scheduled_searches.json'"], [None, 'DEFAULT_INTERVAL_HOURS = 6.0'], [None, 'def load_schedules() -> List[Dict]:'], [None, 'def save_schedules(schedules: Dict[str, Dict]) -> None:'], [None, 'def upsert_schedule(sched: Dict) -> None:'], [None, 'def is_due(sched: Dict) -> bool:'], [None, 'def effective_programs(sched: Dict) -> List[str]:'], [None, 'def query_legs(sched: Dict) -> List[Dict]:']]
