"""Adversarial fixture for: aw-airport-groups-s1-is-group
...
CASES = [
    ...
    ("JFK is a single airport, not a group", lambda: target.is_group("JFK"), False),
    ("LAX is a single airport, not a group", lambda: target.is_group("LAX"), False),
    # unknown code: catches "any 3-letter alpha string" impls
    ("unknown three-letter code is not a group", lambda: target.is_group("ZZZ"), False),
    # degenerate inputs must return False, never raise
    ("empty string returns False without raising", lambda: target.is_group(""), False),
    ("None returns False without raising", lambda: target.is_group(None), False),
    ("non-string (int) returns False without raising", lambda: target.is_group(12345), False),
]
