#!/usr/bin/env bash
# verify for: aw-app-wiring-s3-from-import-scheduled-ro
...
echo "=== target parses ==="
if "$PY" -c "import ast,sys; ast.parse(open('src/webui/app.py').read())"; then
  echo "  ok: src/webui/app.py parses"
else
  echo "  FAIL: src/webui/app.py does not parse"; fails=$((fails+1))
fi
