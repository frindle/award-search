#!/usr/bin/env bash
# verify for: aw-transfer-partners-s10-restore-amex-mr-chase-ur
...
echo "=== target parses ==="
if "$PY" -c "import ast,sys; ast.parse(open('src/transfer_partners.py').read())"; then
  echo "  ok: src/transfer_partners.py parses"
else
  echo "  FAIL: src/transfer_partners.py does not parse"; fails=$((fails+1))
fi
