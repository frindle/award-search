#!/usr/bin/env bash
# verify for: aw-alert-filters-s1-filter-dict-airlines-ua
...
echo "=== target parses ==="
if "$PY" -c "import ast,sys; ast.parse(open('src/alert_filters.py').read())"; then
  echo "  ok: src/alert_filters.py parses"
else
  echo "  FAIL: src/alert_filters.py does not parse"; fails=$((fails+1))
fi
