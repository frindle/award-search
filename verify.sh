#!/usr/bin/env bash
# verify for: aw-scheduled-searches-s3-cabins-business-first-pr
...
echo "=== target parses ==="
if "$PY" -c "import ast,sys; ast.parse(open('src/scheduled_searches.py').read())"; then
  echo "  ok: src/scheduled_searches.py parses"
else
  echo "  FAIL: src/scheduled_searches.py does not parse"; fails=$((fails+1))
fi
