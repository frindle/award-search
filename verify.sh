#!/usr/bin/env bash
# verify for: aw-sched-routes-s18-get-api-transfer-partner
...
echo "=== target parses ==="
if "$PY" -c "import ast,sys; ast.parse(open('src/webui/scheduled_routes.py').read())"; then
  echo "  ok: src/webui/scheduled_routes.py parses"
else
  echo "  FAIL: src/webui/scheduled_routes.py does not parse"; fails=$((fails+1))
fi
