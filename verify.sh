#!/usr/bin/env bash
# verify for: aw-airport-groups-s2-expand-codes
...
echo "=== target parses ==="
if "$PY" -c "import ast,sys; ast.parse(open('src/airport_groups.py').read())"; then
  echo "  ok: src/airport_groups.py parses"
else
  echo "  FAIL: src/airport_groups.py does not parse"; fails=$((fails+1))
fi
